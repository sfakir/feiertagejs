...
describe('Check Feiertage by Name', () => {
  ...
  it('check ChristiHimmelfahrt 2016: check wrong dates', () => {
    ...
    let result = feiertagejs.isSpecificHoliday(
      new Date(2016, 9, 15),
      feiertagejs.Holidays.CHRISTIHIMMELFAHRT
    );
    expect(result).to.be.an('boolean');
    expect(result).to.equal(false);

    result = feiertagejs.isSpecificHoliday(
      new Date(2016, 12, 24),
      feiertagejs.Holidays.CHRISTIHIMMELFAHRT
    );
    expect(result).to.be.an('boolean');
  ...
  it('check ChristiHimmelfahrt 2016: check right date', () => {
    ...
    const result = feiertagejs.isSpecificHoliday(
      somedate,
      feiertagejs.Holidays.CHRISTIHIMMELFAHRT
    ); // und Vatertag
    expect(result).to.be.an('boolean');
  ...
  it('check Erster Weihnachtsfeiertag 2016: check right date', () => {
    ...
    const result = feiertagejs.isSpecificHoliday(
      somedate,
      feiertagejs.Holidays.ERSTERWEIHNACHTSFEIERTAG
    );
    expect(result).to.be.an('boolean');
    expect(result).to.equal(true);
  });
