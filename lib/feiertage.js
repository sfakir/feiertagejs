...
(function () {
     ...
     * @private
     */
    feiertagejs._holidays = [];
    //extract keys only
    for (var key in feiertagejs.translation['de']) {
        //feiertagejs._holidays.push(key);
        feiertagejs._holidays[key] = key;
    }
    ...
    function isSpecificHoliday(date, holidayName, region) {
        ...
        if (feiertagejs._holidays[holidayName] === null) {
        ...
        return (_getGetNormalizedDate(date) == foundHoliday.date.getNormalizedDate());
    }
     ...
     * @returns { integers: [int,int,int,..], objects:[{date:Date, name: String}] }
     */
...
// Erweitert Date um eine addDays()-Methode
     ...
     *
     * @param name
     * @param year integer|date
     * @param realMonth integer | null
    ...
    function _compareDates(date1, date2) {
        return (_dateObjectToInternalDate(date1) == _dateObjectToInternalDate(date2));
    }
