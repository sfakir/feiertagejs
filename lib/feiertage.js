...
(function() {
  ...
  function setLanguage(isoCode) {
    ...
    if (!feiertagejs.translation[isoCode]) {
      if (env !== 'test') {
        console.error(
          '[feiertagejs] tried to set lanague to ',
          isoCode,
  ...
  function isSpecificHoliday(date, holidayName, region) {
    ...
    if (feiertagejs.Holidays[holidayName] === null) {
      throw new Error(
        '[Feiertage.js] PLEASE USE THE CONSTANTS FROM feiertagejs.Holidays.xx'
      );
    }
  ...
  function _getHolidaysOfYear(j, region) {
    ...
    var feiertageObjects = [
      _newHoliday('NEUJAHRSTAG', j, 1, 1),
      _newHoliday('TAG_DER_ARBEIT', j, 5, 1),
      _newHoliday('DEUTSCHEEINHEIT', j, 10, 3),
      _newHoliday('ERSTERWEIHNACHTSFEIERTAG', j, 12, 25),
      _newHoliday('ZWEITERWEIHNACHTSFEIERTAG', j, 12, 26)
    ];
    ...
    feiertageObjects.unshift(
      _newHoliday('CHRISTIHIMMELFAHRT', christi_himmelfahrt)
    );
